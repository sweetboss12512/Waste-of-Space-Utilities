...
function Communication.SubscribeToTopic(topicName: string, port: PilotLuaPortLike & any, callback)
...
function Communication.SubscribeToAntennaScramble(port: PilotLuaPart | number)
	local antenna = GetPartFromPort(port, "Antenna") or error("[Communication.ScrambleAntennaID]: No antenna found on the provided port")
